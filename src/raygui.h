...
int GuiTextBox(Rectangle bounds, char *text, int textSize, bool editMode)
{
        ...
        (wrapMode == TEXT_WRAP_NONE))               // No wrap mode
    {
        ...
        if (editMode)
        {
            ...
            // NOTE: textIndexOffset is bounded by textLength so a degenerate (<=0 width) box
            // can't walk the pointer past the buffer's NUL into unmapped memory (SIGBUS).
            while ((textWidth >= textBounds.width) && (textIndexOffset < textLength))
            {
