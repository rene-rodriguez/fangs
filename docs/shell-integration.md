...
Fangs's VT engine (libghostty-vt) tracks **OSC 133 semantic prompt marks**: it can
...
```zsh
# Fangs OSC 133 shell integration
autoload -Uz add-zsh-hook
_fangs_precmd()  { print -Pn "\e]133;D;$?\e\\"; print -Pn "\e]133;A\e\\"; }  # prev cmd done + prompt start
_fangs_preexec() { print -Pn "\e]133;C\e\\"; }                               # command about to run
...
PROMPT_COMMAND="_fangs_prompt${PROMPT_COMMAND:+; $PROMPT_COMMAND}"
PS1="${PS1}\[\e]133;B\e\\\\\]"     # prompt end / command start (B mark)
# "C" (command running) via the DEBUG trap, skipping the prompt hook itself:
_fangs_preexec() { [[ "$BASH_COMMAND" == _fangs_prompt ]] || printf '\e]133;C\e\\'; }
trap '_fangs_preexec' DEBUG
```

After editing, `source ~/.zshrc` (or `~/.bashrc`) or open a new Fangs window.
