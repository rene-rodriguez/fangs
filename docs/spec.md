# Nova Terminal — Technical Specification
...
This document specifies *how* Nova Terminal is built: module contracts, data flows, the config
...
- File: `~/.config/nova-terminal/config` (INI). **Created by the app on first run if absent.** Not in the repo; user-owned.
- **API key precedence:** `NOVA_API_KEY` env var **wins**; the `[ai] api_key` file field is a fallback. This keeps the key out of the file by default and avoids accidental commits. The settings modal writes to the file only if the user explicitly enters a key there.
...
model       = gpt-4o-mini
api_key     =                 ; leave blank; prefer NOVA_API_KEY env var
stream      = true
max_tokens  = 1024
temperature = 0.2
...
- **Key handling:** prefer `NOVA_API_KEY` env var; if stored in the INI, the file is `~/.config/nova-terminal/config` (user-owned, mode `0600` enforced by the app on write). Never logged, never sent anywhere but the provider.
...
```bash
$EDITOR ~/.config/nova-terminal/config
export NOVA_API_KEY=sk-...      # preferred over putting the key in the file
```
...
|---|---|---|
| **0a** | Pinned engine builds | `./build/nova-terminal` opens a terminal window from a pinned `libghostty-vt` SHA on CachyOS |
| **0b** | Streaming AI spike | Tokens from one hardcoded prompt visibly stream into a Raylib window via worker thread + ring buffer |
| **1** ✅ | Daily-drivable terminal | **DONE** — `src/{main,pty,term_engine}.c` + root `CMakeLists.txt`; builds → `build/nova-terminal`, runs clean (ghostty-vt + raylib/Cocoa/OpenGL). Engine behind the `term_engine` seam. Handoff: `docs/handoff-phase2.md` |
...
- ~~Packaging target: AUR `PKGBUILD` (CachyOS) + `.app`/Homebrew (macOS) vs. `cmake --install`.~~ **Resolved (2026-06-22):** AUR `PKGBUILD` shipped + verified (`packaging/aur/`, `nova-terminal-git`); macOS `.app`/Homebrew still TODO.
