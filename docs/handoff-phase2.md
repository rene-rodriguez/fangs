...
**Build & iterate**
- First/clean build (macOS): `bash scripts/macos-build.sh` → `build/fangs`.
...
A `~/.config/fangs/config` INI is the source of truth for terminal + AI settings;
...
#include "raygui.h"
```
No CMake change needed beyond adding `src/ui_settings.c` to the `add_executable(fangs ...)` list.
...
typedef struct {
    ...
    char  endpoint[256];
    char  model[128];
    char  api_key[256];       // usually empty; env FANGS_API_KEY wins (see below)
    bool  stream;
    int   max_tokens;
} AppConfig;
...
const char *config_default_path(void);  // "$HOME/.config/fangs/config" (mkdir -p the dir)
```
- **API key precedence:** at the point of use (Phase 4), `getenv("FANGS_API_KEY")` wins;
...
- [ ] `~/.config/fangs/config` is created with defaults on first run.
...
- [ ] `cmake --build build` stays warning-clean; `./build/fangs` runs `vim`/`htop` fine.
