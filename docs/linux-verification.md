...
> **Purpose:** the one remaining v1 item — confirm Nova's features **on-screen on a
...
`ai-terminal-ghosttly` → `nova-terminal`, which can poison an old cache).

```sh
cd ~/…/nova-terminal      # repo root
...
NOVA_BLOCKS_SMOKE_SCREENSHOT=/tmp/nova_blocks.png ./build/nova-terminal
# Split layout + sidebar:
NOVA_PHASE3_SMOKE_SCREENSHOT=/tmp/nova_phase3.png ./build/nova-terminal
```

If the box is headless but you want the smoke anyway: `xvfb-run -a env NOVA_BLOCKS_SMOKE_SCREENSHOT=/tmp/nova_blocks.png ./build/nova-terminal`.
...
```sh
export NOVA_API_KEY=…          # for the live AI step below (cloud providers)
./build/nova-terminal
```

**No cloud key? Use a local Ollama model.** Install Ollama, pull a chat model, and point
Nova at it — no key needed:
...
# Or edit ~/.config/nova-terminal/config directly:
#   [ai]
#   provider = ollama
#   endpoint = http://localhost:11434/v1/chat/completions
#   model    = qwen2.5:7b
#   api_key  = ollama        # any non-empty value; Ollama ignores it, Nova just needs it non-blank
```
...
- [ ] **Live AI request** — with `NOVA_API_KEY` set, ask the sidebar about something
...
_nova_precmd()  { print -Pn "\e]133;D;$?\e\\"; print -Pn "\e]133;A\e\\"; }
_nova_preexec() { print -Pn "\e]133;C\e\\"; }
add-zsh-hook precmd  _nova_precmd
add-zsh-hook preexec _nova_preexec
PS1="$ %{$(print -Pn '\e]133;B\e\\')%}"
EOF
SHELL=$(command -v zsh) ZDOTDIR="$ZD" ./build/nova-terminal
```
...
- mem0 may surface stale **"nova-cockpit" / TypeScript** memories — ignore; this is the
  native C repo. Serena registers it under the old name `ai-terminal-ghosttly`.
- **HiDPI on Wayland fractional scaling.** Nova multiplies `font_size` by the display's content
  ...
  **Wayland** backend reports ~**1.0** even at e.g. **1.5×**, which used to render the font tiny.
  `nova_content_scale()` now **auto-detects** the scale from the monitor's physical DPI (`DPI/96`,
  ...
  - **`NOVA_SCALE`** wins over auto-detect — `NOVA_SCALE=1.5 nova-terminal` (env). Check a
