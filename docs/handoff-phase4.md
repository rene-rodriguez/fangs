...
for a clean/reconfigure build; `ctest --test-dir build --output-on-failure`. Run: `./build/fangs`.
...
typedef struct {
    ...
    const char *api_key;      // RESOLVED: getenv("FANGS_API_KEY") ?: cfg.api_key
...
- Move `spike/ai_stream/cJSON.{c,h}` → `src/` (or `vendor/cjson/`). Add `src/cJSON.c` to the
  `fangs` target.
- libcurl: `find_package(CURL REQUIRED)` then `target_link_libraries(fangs CURL::libcurl)`.
...
if (ui_sidebar_draw(lo.sidebar, submitted, sizeof submitted)) {
    ...
    if (!active_stream) ui_sidebar_push(MSG_SYSTEM, "No API key — set FANGS_API_KEY or add one in Ctrl+,");
...
```
- `resolve_key(&cfg)`: `const char *k = getenv("FANGS_API_KEY"); return (k && *k) ? k : cfg.api_key;`
