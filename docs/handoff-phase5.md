...
`bash scripts/macos-build.sh` for a clean build. Run: `./build/nova-terminal`.
...
1. **Intercept `Ctrl+Space`** before PTY input, beside the existing `Ctrl+,` / `Cmd+B` blocks:
   ...
   document that Nova reserves it. If a user needs literal `Ctrl+Space`, that's a known tradeoff.)
