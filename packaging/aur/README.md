# AUR packaging — `nova-terminal-git`

A VCS `PKGBUILD` that builds Nova Terminal from the `main` branch for
...
- `nova-terminal.desktop` — application launcher entry.
...
- **Repo must be reachable.** The `source` clones
  `https://github.com/rene-rodriguez/nova-terminal.git`. While that repo is
